...
def save_quiz_results(user_uid, topics, weeks, question_results, time_taken_seconds):
    ...
    try:
        ...
        correct_answers = sum(1 for q in question_results.values() if q.get('is_correct', False))
...
def update_user_stats(db, user_uid, quiz_id, quiz_data):
    ...
    if user_doc.exists:
        ...
        total_questions = user_stats.get('total_questions_attempted', 0) + quiz_data['num_questions']
        total_correct = user_stats.get('total_correct_answers', 0) + quiz_data['correct_answers']
        total_time = user_stats.get('total_time_spent_seconds', 0) + quiz_data['time_taken_seconds']
        ...
        for question in quiz_data['questions']:
            ...
            if quiz_data['topics']:
                topic_id = str(quiz_data['topics'][0])  # Convert to string for Firestore keys
                ...
                topic_performance[topic_id]['average_score'] = (topic_correct / topic_questions) * 100
...
def get_quiz_history(user_uid, limit=10):
    ...
    try:
        ...
        try:
            ...
            query = quiz_ref.where('user_uid', '==', user_uid).order_by('date', direction=firestore.Query.DESCENDING).limit(limit)
            quiz_history = []
            ...
            st.warning("Firestore index not yet created. Using fallback method to retrieve quiz history.")
            st.info("To improve performance, please create the suggested index in the Firebase console.")
            ...
            if all_quizzes:
                ...
                if all_quizzes[0].get('date') and hasattr(all_quizzes[0]['date'], 'timestamp'):
                    # Sort by timestamp
                    all_quizzes.sort(key=lambda x: x.get('date').timestamp() if x.get('date') else 0, reverse=True)
                else:
...
def check_and_award_badges(user_uid):
    ...
    try:
        ...
        badge_criteria = {
            ...
            'perfect_score': lambda stats: any(q.get('score', 0) == 100 for q in get_quiz_history(user_uid)),
            'streak_3': lambda stats: stats.get('streak_days', 0) >= 3,
            'streak_7': lambda stats: stats.get('streak_days', 0) >= 7,
            'hundred_questions': lambda stats: stats.get('total_questions_attempted', 0) >= 100,
    ...
    if quiz_history:
        for i, quiz in enumerate(quiz_history):
            ...
            if topic_ids:
                ...
                topic_names_list = [topic_names_dict.get(str(tid), f"Topic {tid}") for tid in topic_ids]
            ...
            with st.expander(f"Quiz {i+1}: {date_str} - Score: {score:.1f}%"):
                ...
                if st.checkbox(f"Show Question Details", key=f"quiz_{i}_details"):
                    questions = quiz.get('questions', [])
                    for j, q in enumerate(questions):
                        is_correct = q.get('is_correct', False)
                        icon = "✅" if is_correct else "❌"
                        st.markdown(f"{icon} Question {j+1}: User answered {q.get('user_answer')}, Correct answer: {q.get('correct_answer')}")
    else:
