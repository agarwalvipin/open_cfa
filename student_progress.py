...
def main():
    ...
    with tab1:
        ...
        avg_score = sum(
            s["average_score"] for s in student_stats if s["total_quizzes"] > 0
        ) / max(1, sum(1 for s in student_stats if s["total_quizzes"] > 0))
            ...
            [
                {
                    ...
                    if s["total_quizzes"] > 0
                    else "N/A",
                    "Last Active": f"{s['days_since_active']} days ago"
                    if s["days_since_active"] < 365
                    else "Never",
        ...
        selected_student = st.data_editor(
            ...
            column_config={
                ...
                "Quizzes Taken": st.column_config.NumberColumn(
                    "Quizzes", width="small",
                ),
                "Avg Score": st.column_config.TextColumn("Avg Score", width="small"),
                "Last Active": st.column_config.TextColumn(
                    "Last Active", width="small",
                ),
                "Current Streak": st.column_config.NumberColumn(
                    "Streak", width="small",
                ),
        ...
        if selected_student is not None and "View" in selected_student:
            selected_uid = (
                selected_student["View"].iloc[0]
                if not selected_student["View"].empty
                else None
            )
            if selected_uid:
    ...
    with tab2:
        ...
        if "selected_student_uid" in st.session_state:
            ...
            selected_student = next(
                (s for s in student_stats if s["uid"] == selected_uid), None,
            )

            if selected_student:
                st.subheader(
                    f"Student: {selected_student['displayName']} ({selected_student['email']})",
                )
                ...
                col3.metric(
                    "Average Score",
                    f"{selected_student['average_score']:.1f}%"
                    if selected_student["total_quizzes"] > 0
                    else "N/A",
                )
                    ...
                    with col1:
                        ...
                        if weak_areas:
                            for topic_id in weak_areas:
                                ...
                                score = topic_performance.get(topic_id, {}).get(
                                    "average_score", 0,
                                )
                ...
                if quiz_history:
                    ...
                    quiz_df = pd.DataFrame(
                        [
                            {
                                "Date": quiz.get("date").strftime("%Y-%m-%d %H:%M")
                                if isinstance(quiz.get("date"), datetime)
                                else "Unknown",
