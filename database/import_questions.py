...
def import_questions_to_db(questions, conn):
    ...
    for question in questions:
        ...
        week_id = None
        if question['week']:
            cursor.execute("SELECT week_id FROM weeks WHERE week_number = %s", (question['week'],))
            result = cursor.fetchone()
...
def main():
    parser = argparse.ArgumentParser(description='Import CFA questions from markdown files into PostgreSQL')
    parser.add_argument('--directory', '-d', required=True, help='Directory containing question markdown files')
