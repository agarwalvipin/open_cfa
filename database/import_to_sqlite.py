...
def import_questions_to_db(questions, conn):
    ...
    for question in questions:
        ...
        if question["week"]:
            cursor.execute(
                "SELECT week_id FROM weeks WHERE week_number = ?", (question["week"],),
            )
            result = cursor.fetchone()
...
def main():
    parser = argparse.ArgumentParser(
        description="Import CFA questions from markdown file into SQLite",
    )
    parser.add_argument(
        "--file", "-f", required=True, help="Markdown file containing questions",
    )
    parser.add_argument(
        "--db", "-d", default="cfa_questions.db", help="SQLite database file to create",
    )
    parser.add_argument(
        "--schema",
        "-s",
        default="sqlite_schema.sql",
        help="Schema file for SQLite database",
    )
