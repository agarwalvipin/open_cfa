...
def get_all_questions(conn, limit=100, offset=0, search_term=None, topic_id=None, week_id=None):
...
def main():
    ...
    with tab1:
        ...
        with col1:
            search_term = st.text_input("Search in title or question text", key="search_term")
        
        with col2:
            # Get topics for filter
            topics = get_topics(conn)
            topic_options = [(None, "All Topics")] + [(t['topic_id'], t['name']) for t in topics]
            selected_topic = st.selectbox(
                "Filter by Topic",
                options=[t[0] for t in topic_options],
                format_func=lambda x: next((t[1] for t in topic_options if t[0] == x), "All Topics"),
                key="topic_filter"
            )
        ...
        with col3:
            ...
            week_options = [(None, "All Weeks")] + [(w['week_id'], f"Week {w['week_number']}") for w in weeks]
            selected_week = st.selectbox(
                "Filter by Week",
                options=[w[0] for w in week_options],
                format_func=lambda x: next((w[1] for w in week_options if w[0] == x), "All Weeks"),
                key="week_filter"
            )
        ...
        if questions:
            ...
            selected_rows = st.data_editor(
                questions_df[['question_id', 'title', 'topic', 'week', 'difficulty', 'num_options', 'view']],
                column_config={
                    ...
                    "num_options": st.column_config.NumberColumn("Options", width="small"),
                    "view": st.column_config.CheckboxColumn("Select", width="small"),
                },
                hide_index=True,
                use_container_width=True,
                disabled=["question_id", "title", "topic", "week", "difficulty", "num_options"]
            )
            
            # Handle selected question
            if selected_rows is not None and 'view' in selected_rows:
                selected_question_ids = selected_rows[selected_rows['view']]['question_id'].tolist()
            ...
            with col3:
                if len(questions) == items_per_page:  # If we have a full page, there might be more
    ...
    with tab2:
        ...
        if 'selected_question_id' in st.session_state:
            ...
            if question_data:
                ...
                st.subheader(f"Question ID: {question_id} - {question_data['question']['title']}")
                ...
                with col2:
                    st.write(f"**Difficulty:** {question_data['question']['difficulty']}")
                ...
                with st.expander("Edit Question"):
                    st.warning("Editing functionality will be implemented in a future update.")
                    
                    # Placeholder for edit form
                    st.text_input("Title", value=question_data['question']['title'])
                    st.text_area("Question Text", value=question_data['question']['question_text'])
