...
import user_stats
from admin_dashboard import cleanup_questions, get_system_stats, get_user_activity_stats
...
def main():
            ...
            if isinstance(auth_param, list) and len(auth_param) > 0:
                ...
                if is_auth_token_valid(auth_token):
                    if restore_session_from_token(auth_token):
                        st.success(
                            f"Welcome back, {st.session_state.username}! You've been automatically logged in.",
                        )
                else:
    ...
    if (
        ...
        and st.session_state.saved_email
        and not st.session_state.authenticated
    ):
        st.info(
            f"Welcome back! Using saved credentials for {st.session_state.saved_email}",
        )
                    ...
                    if email and password:
                        ...
                        try:
                            ...
                            if result["success"]:
                                ...
                                st.session_state["user_uid"] = result["user_info"][
                                    "users"
                                ][0]["localId"]
                                ...
                                st.success(
                                    f"Login successful! You are logged in as a {user_role.capitalize()}.",
                                )
    ...
    if st.session_state.current_page == "dashboard":
                ...
                with col2:
                    if st.button(
                        ...
                        try:
                            ...
                            # Success message
                            st.success(
                                f"Successfully imported {len(questions)} questions from URL!",
                            )
        ...
        tab1, tab2, tab3 = st.tabs(
            ["Question Database", "User Activity", "Database Management"],
        )
        ...
        with tab1:
            ...
            with col1:
                ...
                if system_stats["questions_by_topic"]:
                    topic_data = [
                        dict(row) for row in system_stats["questions_by_topic"]
                    ]
                ...
                if system_stats["questions_by_difficulty"]:
                    difficulty_data = [
                        dict(row) for row in system_stats["questions_by_difficulty"]
                    ]
        ...
        with tab2:
            ...
            if user_activity["recent_quizzes"]:
                ...
                if "date" in quiz_df.columns:
                    quiz_df["date"] = quiz_df["date"].apply(
                ...
                if "time_taken_seconds" in quiz_df.columns:
                    quiz_df["time_taken"] = quiz_df["time_taken_seconds"].apply(
                        lambda x: f"{x // 60}m {x % 60}s",
                    )
                ...
                st.dataframe(
                    quiz_df[
                        ["date", "user_uid", "num_questions", "score", "time_taken"]
                    ],
                    use_container_width=True,
                )
            else:
        ...
        with tab3:
            ...
            st.warning(
                "⚠️ Warning: This operation will permanently delete questions and related data from the database. This action cannot be undone.",
            )
            ...
            with col2:
                # Week filter
                cursor.execute(
                    "SELECT week_id, week_number FROM weeks ORDER BY week_number",
                )
            ...
            st.info(
                f"This operation will delete {affected_count} questions and their related data.",
            )

            # Confirmation checkbox
            confirm = st.checkbox(
                "I understand that this operation cannot be undone and confirm the deletion",
            )
            ...
            st.info(
                "This will remove unused topics, tags, modules, and other reference data that are not associated with any questions.",
            )
            ...
            if st.button("Clean Orphaned Data", type="secondary"):
                try:
                    ...
                    st.success(
                        f"Successfully cleaned up orphaned data: {tags_deleted} tags, {los_deleted} learning objectives, {modules_deleted} modules, and {readings_deleted} readings.",
                    )
        ...
        if st.session_state.quiz_wizard_step == 1:
            ...
            if not topic_options:
                st.error(
                    "No topics found in the database. Please import questions first.",
                )
                st.stop()
            ...
            if len(topic_options) == 1:
                ...
                selected_topic_names = st.multiselect(
                    ...
                    default=["All"]
                    if not st.session_state.quiz_filters["topic_ids"]
                    else [
                        name
                        for name in topic_options.keys()
                        if topic_options[name]
                        in st.session_state.quiz_filters["topic_ids"]
                    ],
            ...
            if "All" in selected_topic_names:
                ...
                selected_topic_ids = [
                    topic_options[name] for name in selected_topic_names
                ]
        ...
        # Step 2: Week Selection
            ...
            weeks = cursor.fetchall()
            week_options = {
                f"Week {week['week_number']}": week["week_id"] for week in weeks
            }
            ...
            if len(week_options) == 1:
                ...
                selected_week_names = st.multiselect(
                    ...
                    default=["All"]
                    if not st.session_state.quiz_filters["week_ids"]
                    else [
                        name
                        for name in week_options.keys()
                        if week_options[name]
                        in st.session_state.quiz_filters["week_ids"]
                    ],
        ...
        # Step 3: Difficulty Selection
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT difficulty_id, name FROM difficulty_levels ORDER BY difficulty_id",
            )
            difficulties = cursor.fetchall()
            difficulty_options = {
                diff["name"]: diff["difficulty_id"] for diff in difficulties
            }
            ...
            if difficulties:
                selected_difficulty_names = st.multiselect(
                    ...
                    default=["All"]
                    if not st.session_state.quiz_filters["difficulty_ids"]
                    else [
                        name
                        for name in difficulty_options
                        if difficulty_options[name]
                        in st.session_state.quiz_filters["difficulty_ids"]
                    ],
                ...
                if "All" in selected_difficulty_names:
                    ...
                    selected_difficulty_ids = [
                        difficulty_options[name] for name in selected_difficulty_names
                    ]

                # Store selected difficulties in session state
                st.session_state.quiz_filters["difficulty_ids"] = (
                    selected_difficulty_ids
                )
            else:
        ...
        # Step 4: Module Selection
            ...
            if modules:
                selected_module_names = st.multiselect(
                    ...
                    default=["All"]
                    if not st.session_state.quiz_filters["module_ids"]
                    else [
                        name
                        for name in module_options
                        if module_options[name]
                        in st.session_state.quiz_filters["module_ids"]
                    ],
                ...
                if "All" in selected_module_names:
                    ...
                    selected_module_ids = [
                        module_options[name] for name in selected_module_names
                    ]
        ...
        # Step 5: Tags Selection and Quiz Size
            ...
            if tags:
                selected_tag_names = st.multiselect(
                    ...
                    default=["All"]
                    if not st.session_state.quiz_filters["tag_ids"]
                    else [
                        name
                        for name in tag_options
                        if tag_options[name] in st.session_state.quiz_filters["tag_ids"]
                    ],
                ...
                if "All" in selected_tag_names:
                    ...
                    selected_tag_ids = [
                        tag_options[name] for name in selected_tag_names
                    ]
            ...
            if st.session_state.quiz_filters["topic_ids"]:
                topic_ids_str = ",".join(
                    "%s" for _ in st.session_state.quiz_filters["topic_ids"]
                )
                conditions.append(f"q.topic_id IN ({topic_ids_str})")
            ...
            if st.session_state.quiz_filters["week_ids"]:
                week_ids_str = ",".join(
                    "%s" for _ in st.session_state.quiz_filters["week_ids"]
                )
                conditions.append(f"q.week_id IN ({week_ids_str})")
                params.extend(st.session_state.quiz_filters["week_ids"])

            if st.session_state.quiz_filters["difficulty_ids"]:
                difficulty_ids_str = ",".join(
                    "%s" for _ in st.session_state.quiz_filters["difficulty_ids"]
                )
            ...
            if st.session_state.quiz_filters["module_ids"]:
                module_ids_str = ",".join(
                    "%s" for _ in st.session_state.quiz_filters["module_ids"]
                )
            ...
            if st.session_state.quiz_filters["tag_ids"]:
                ...
                tag_ids_str = ",".join(
                    "%s" for _ in st.session_state.quiz_filters["tag_ids"]
                )
            ...
            st.info(
                f"Total available questions with selected filters: {question_count}",
            )
            ...
            if question_count > 0:
                ...
                if st.button(
                    ...
                    if st.session_state.quiz_filters["difficulty_ids"]:
                        difficulty_ids_str = ",".join(
                            "%s"
                            for _ in st.session_state.quiz_filters["difficulty_ids"]
                        )
                        conditions.append(f"q.difficulty_id IN ({difficulty_ids_str})")
                        params.extend(st.session_state.quiz_filters["difficulty_ids"])
                ...
                st.info(
                    "Try selecting different filters or import more questions into the database.",
                )
    ...
    if st.session_state.current_page not in [
        "dashboard",
        "load_questions",
    ] and selection_mode in ["Topic", "Both"]:
        # Add 'All' option
        topic_options_with_all = {"All": "all"} | topic_options
    ...
    if st.session_state.current_page not in [
        "dashboard",
        "load_questions",
    ] and selection_mode in ["Week", "Both"]:
        # Add 'All' option
        week_options_with_all = {"All": "all"} | week_options
    ...
    if "All" in selected_week_names:
        ...
        selected_week_ids = [
            week_options[name] for name in selected_week_names if name in week_options
        ]
    ...
    if selection_mode == "Topic" and selected_topic_ids:
        ...
        selection_mode == "Topic"
        and len(selected_topic_names) > 0
        and "All" in selected_topic_names
    ):
        has_selection = True
        question_count = get_question_count(
        ...
        selection_mode == "Week"
        and len(selected_week_names) > 0
        and "All" in selected_week_names
    ):
        has_selection = True
        question_count = get_question_count(conn, week_ids=list(week_options.values()))
        st.sidebar.info(f"Total available questions: {question_count}")
    elif selection_mode == "Both" and (
        (len(selected_topic_names) > 0 and "All" in selected_topic_names)
        or (len(selected_week_names) > 0 and "All" in selected_week_names)
    ):
        has_selection = True
        topic_ids = (
    ...
    if has_selection and question_count > 0:
        ...
        if selection_mode == "Topic":
            ...
            st.info(
                "👈 Please select at least one topic or week from the sidebar to start.",
            )
    else:
        st.warning("No questions available with the current selection.")
        st.info(
            "Currently, only ethics questions from Week 3 have been imported into the database.",
        )
        st.info(
            "More questions will be available as they are imported from the question bank.",
        )
    ...
    if "quiz_started" in st.session_state and st.session_state.quiz_started:
        if not st.session_state.show_results:
            ...
            st.subheader(
                f"Your Score: {correct_answers}/{total_questions} ({score_percentage:.1f}%)",
            )
            if time_info:
                st.write(time_info)
                    ...
                    if new_badges:
                        ...
                        for badge_id in new_badges:
                            badge = badge_details.get(
                                badge_id,
                                {"name": badge_id, "description": "", "icon": "🏆"},
                            )
                            st.markdown(
            ...
            for i, q_id in enumerate(st.session_state.question_ids):
                ...
                with st.expander(
                    f"Question {i + 1}: {question_data['question']['title']}",
                ):
                    st.write(question_data["question"]["question_text"])
                    ...
                    for option in question_data["options"]:
                        if q_id in st.session_state.user_answers:
                            ...
                            if option["option_letter"] == user_answer:
                                ...
                                st.info(
                                    f"{option['option_letter']}) {option['option_text']} (Correct Answer)",
                                )
                            else:
                                st.write(
                                    f"{option['option_letter']}) {option['option_text']}",
    ...
    # Practice Question page
        ...
        if not st.session_state.practice_question_answered:
            if st.button(
                ...
                if selection_mode == "Topic":
                    ...
                    questions = get_filtered_questions(
                        conn,
                        topic_ids=topic_ids_for_query,
                        week_ids=week_ids_for_query,
                        limit=1,
                    )
        ...
        if st.session_state.practice_current_question:
            ...
            if question_data:
                ...
                if not st.session_state.practice_question_answered:
                    for option in question_data["options"]:
                        option_key = (
                            f"practice_q{question_id}_{option['option_letter']}"
                        )
                        if st.button(
                            f"{option['option_letter']}) {option['option_text']}",
                            key=option_key,
                        ):
                            st.session_state.practice_user_answer = option[
                                "option_letter"
                            ]
                            ...
                            if "user" in st.session_state:
                                ...
                                question_results = {
                                    question_id: {
                                        ...
                                        "is_correct": st.session_state.practice_user_answer
                                        == correct_answer,
                                    },
                                }
                                ...
                                # Save result
                                topic_ids = (
                                    selected_topic_ids if selected_topic_ids else []
                                )
                                week_ids = (
                                    selected_week_ids if selected_week_ids else []
                                )
        ...
        if (
            not st.session_state.practice_current_question
            and not st.session_state.practice_question_answered
        ):
            st.info(
                "Select your filters in the sidebar and click 'Get a New Question' to start practicing.",
            )
