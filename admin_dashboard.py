...
def cleanup_questions(conn, topic_id=None, week_id=None, confirm=False):
    ...
    try:
        ...
        cursor.execute(
            f"DELETE FROM question_tags WHERE question_id IN ({placeholders})",
            question_ids,
        )
        ...
        cursor.execute(
            f"DELETE FROM explanations WHERE question_id IN ({placeholders})",
            question_ids,
        )
        ...
        cursor.execute(
            f"DELETE FROM answer_options WHERE question_id IN ({placeholders})",
            question_ids,
        )
        ...
        return (
            f"Successfully deleted {questions_deleted} questions, {options_deleted} answer options, {explanations_deleted} explanations, and {tags_deleted} question tags.",
            True,
        )
...
def main():
    ...
    with tab2:
        ...
        if user_activity["recent_quizzes"]:
            ...
            st.dataframe(
                quiz_df[["date", "user_uid", "num_questions", "score", "time_taken"]],
